...
function activate(context) {
	...
	let colorPicker = vscode.commands.registerCommand('colorbar.pick', () => {

		vscode.window.showQuickPick(keys, {
			onDidSelectItem: item => {
				updateColorCustomizations(item);
			}
		}).then(selection => {
			...
			updateColorCustomizations(selection);

		});
	});

	let editColors = vscode.commands.registerCommand('colorbar.edit', () => {
		vscode.commands.executeCommand('workbench.action.openSettings', '@ext:weezah.colorbar');
	});


	context.subscriptions.push(colorPicker, editColors);
}
...
// Credits to https://github.com/PimpTrizkit/PJs/
